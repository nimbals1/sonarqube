...
public class CachingRuleActivator extends RuleActivator {
  ...
  private void populateForChildren(List<QualityProfileDto> res) {
    if (res.isEmpty()) {
      return;
    }
    QualityProfileDto[] qualityProfileDtos = res.toArray(new QualityProfileDto[res.size()]);
    int length = qualityProfileDtos.length;
    for (int i = 0; i < length; i++) {
      childrenByParentKey.put(qualityProfileDtos[i].getKey(), Arrays.asList(Arrays.copyOfRange(qualityProfileDtos, i+1, length)));
    }
  }
}
